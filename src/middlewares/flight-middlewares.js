...
function validateCreateRequest(req, res, next){
    ...
    if(compareTime(req.body.departureTime, req.body.arrivalTime)){
        ErrorResponse.message = "Something went wrong while creating airport";
        ErrorResponse.error = new AppError(["Arrival Time should be greater then departure time"],StatusCodes.BAD_REQUEST);
        return res
            .status(StatusCodes.BAD_REQUEST)
            .json(ErrorResponse)
    }
...
function validateUpdateSeatsRequest(req, res, next){
    if(!req.params.id){
        ...
        ErrorResponse.error = new AppError(["FlightId not found in the incoming request"],StatusCodes.BAD_REQUEST);
        return res
            .status(StatusCodes.BAD_REQUEST)
            .json(ErrorResponse)
    }
